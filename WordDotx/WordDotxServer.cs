        ...
        public void StartCreateReport(TaskWord Tsk)
        {
            try
            {
                ...
                lock (obj)
                {
                    ...
                    Object pathToSaveObj = (Tsk.Target==null || Tsk.Target.IndexOf(@"\") > 0 ? Tsk.Target : string.Format(@"{0}\{1}", this.DefaultPathTarget, Tsk.Target));
                   ...
                   // }
                    ...
                    for (int ic = 0; ic < itemT.Columns.Count; ic++)
                    {
                        try
                        {
                            ...
                            if (tmpCell.IndexOf("\r") == tmpCell.Length - 1) tmpCell = tmpCell.Substring(0, tmpCell.Length - 1);     // вконце ячейки встаёт системные символы их учитывать не надо
